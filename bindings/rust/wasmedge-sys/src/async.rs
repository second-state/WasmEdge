    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        unsafe {
            ...
            *self.current_poll_cx =
            ...
            ASYNC_CX_TLS.set(&async_cx, || match self.as_ref().fiber.resume(Ok(())) {
...
scoped_tls::scoped_thread_local! {static ASYNC_CX_TLS: AsyncCx}
...
impl AsyncCx {
    ...
    pub unsafe fn block_on<U>(
        ...
        if ASYNC_JMP_BUF.is_set() {
            ASYNC_JMP_BUF.with(|(_env, in_host, _timeout)| {
                **in_host = true;
            })
        }
        ...
        *self.current_suspend = ptr::null();
        ...
        if ASYNC_JMP_BUF.is_set() {
            if ASYNC_JMP_BUF.with(|(_env, in_host, timeout)| {
                **in_host = false;
                **timeout
            }) {
                return Err(());
